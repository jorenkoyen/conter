...
SUPPORTED_TARGETS="darwin_amd64 \
                   darwin_arm64 \
                   linux_amd64 \
                   linux_arm64"
info() {
...
usage() {
  printf "%s\n" \
    "install.sh [option]" \
    "" \
    "Fetch and install the latest version of ${APP_NAME}, if ${APP_NAME} is already" \
...
if [ -z "${BASE_URL-}" ]; then
  BASE_URL="https://github.com/jorenkoyen/conter/releases"
fi
...
if [ "${VERSION}" != "latest" ]; then
  ...
  URL="${BASE_URL}/latest/download/${PACKAGE_NAME}_${TARGET}.${EXT}"
fi


info "Tarball URL: ${UNDERLINE}${BLUE}${URL}${NO_COLOR}"
confirm "Install ${APP_NAME} ${GREEN}${VERSION}${NO_COLOR} to ${BOLD}${GREEN}${BIN_DIR}${NO_COLOR}?"
check_bin_dir "${BIN_DIR}"

install "${EXT}"
completed "${APP_NAME} ${VERSION} installed"
